...
<h5 class="text-dark p-3 border-bottom mt-2"><i class="fas fa-exclamation-triangle"></i>&nbspPerlu Konfirmasi</h5> 
...
<table class="table table-striped table-bordered text-center">
    <thead class="thead-dark">
        <tr>
            <th>No<th>Nama<th>Status<th>Jumlah Peserta<th>Instansi<th>Jurusan<th>Proposal<th>Konfirmasi<th>Detail<th>Hapus
        </tr>
    </thead>
    <tbody>
              <?php
        ...
        if(isset($_POST["cari"])){
            ...
            $filter=$_POST["filter"];
                if ($filter=="id_daftar"){
                $data=mysqli_query($connection,"select * from pendaftaran where id_daftar like '%$cari%' and status='Menunggu Konfirmasi Admin'");
                }elseif($filter=="instansi"){
                $data=mysqli_query($connection,"select * from pendaftaran where instansi like '%$cari%' and status='Menunggu Konfirmasi Admin'");    
                }else{
                $data=mysqli_query($connection,"select * from pendaftaran where nama_ketua like '%$cari%' and status='Menunggu Konfirmasi Admin'");   
                }       
            }else{
            $data=mysqli_query($connection,"select * from pendaftaran where status='Menunggu Konfirmasi Admin'");
        }
        $no=1;
...
<table class="table table-striped table-bordered text-center">
    <thead class="thead-dark">
        <tr>
            <th>No<th>ID Daftar<th>Status<th>Nama<th>Instansi<th>Jurusan<th>Tgl Magang<th>Proposal<th>Detail<th>Hapus
        </tr>
    </thead>
    <tbody>
        <?php
        ...
        if(isset($_POST["cari2"])){
            ...
            $filter2=$_POST["filter2"];
                ...
                $data3=mysqli_query($connection,"select * from pendaftaran where (status='Menunggu konfirm SDM' or status='Menunggu konfirm KABAG SDM' or status='Menunggu konfirm BAGIAN') and id_daftar like '%$cari2%'");
                }elseif($filter2=="instansi"){
                $data3=mysqli_query($connection,"select * from pendaftaran where (status='Menunggu konfirm SDM' or status='Menunggu konfirm KABAG SDM' or status='Menunggu konfirm BAGIAN') and instansi like '%$cari2%'");    
                }elseif($filter2=="nama"){
                $data3=mysqli_query($connection,"sselect * from pendaftaran where (status='Menunggu konfirm SDM' or status='Menunggu konfirm KABAG SDM' or status='Menunggu konfirm BAGIAN') and nama_ketua like '%$cari2%'");    
                }else{
                $data3=mysqli_query($connection,"select * from pendaftaran where (status='Menunggu konfirm SDM' or status='Menunggu konfirm KABAG SDM' or status='Menunggu konfirm BAGIAN')  and status like '%$cari2%'");   
                }
            }else{    
        $data3=mysqli_query($connection,"select * from pendaftaran where status='Menunggu konfirm SDM' or status='Menunggu konfirm KABAG SDM' or status='Menunggu konfirm BAGIAN'");
    }
        $nomor=1;
