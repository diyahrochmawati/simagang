...
<table class="table table-striped table-bordered text-center">
    <thead class="thead-dark">
        <tr>
            <th>No<th>Nama Ketua<th>Jumlah Peserta<th>Instansi<th>Jurusan<th>Tgl Magang<th>Proposal<th>Konfirmasi<th>Detail
        </tr>
    </thead>
    <tbody>
              <?php
        ...
        if(isset($_POST["cari"])){
            ...
            $filter=$_POST["filter"];
                if ($filter=="id_daftar"){
                $data=mysqli_query($connection,"select * from pendaftaran where id_daftar like '%$cari%' and status='Menunggu Konfirm SDM'");
                }elseif($filter=="instansi"){
                $data=mysqli_query($connection,"select * from pendaftaran where instansi like '%$cari%' and status='Menunggu Konfirm SDM'");    
                }else{
                $data=mysqli_query($connection,"select * from pendaftaran where nama_ketua like '%$cari%' and status='Menunggu Konfirm SDM'");   
                }       
            }else{
            $data=mysqli_query($connection,"select * from pendaftaran where status='Menunggu Konfirm SDM'");
        }
        $no=1;
        ...
        <tr>
            ...
            <td>
                <?php echo $data2['tgl_masuk'], ",  ", $data2['tgl_selesai']; ?>
            </td>
...
<table class="table table-striped table-bordered text-center">
    <thead class="thead-dark">
        <tr>
            <th>No<th>Nama Ketua<th>Status<th>Posisi<th>Instansi<th>Jurusan<th>Tgl Magang<th>Proposal<th>Detail
        </tr>
    </thead>
    <tbody>
        <?php
        ...
        if(isset($_POST["cari2"])){
            ...
            $filter2=$_POST["filter2"];
                ...
                $data3=mysqli_query($connection,"select * from pendaftaran where (status='Menunggu Konfirm KABAG SDM' or status='Menunggu Konfirm Bagian') and id_daftar like '%$cari2%'");
                }elseif($filter2=="instansi"){
                $data3=mysqli_query($connection,"select * from pendaftaran where (status='Menunggu Konfirm KABAG SDM' or status='Menunggu Konfirm Bagian') and id_daftar like '%$cari2%'");    
                }elseif($filter2=="nama"){
                $data3=mysqli_query($connection,"sselect * from pendaftaran where (status='Menunggu Konfirm KABAG SDM' or status='Menunggu Konfirm Bagian') and id_daftar like '%$cari2%'"); 
                }else{
                $data3=mysqli_query($connection,"select * from pendaftaran where (status='Menunggu Konfirm KABAG SDM' or status='Menunggu Konfirm Bagian') and id_daftar like '%$cari2%'");
                }
            }else{    
        $data3=mysqli_query($connection,"select * from pendaftaran where status='Menunggu Konfirm KABAG SDM' or status='Menunggu Konfirm Bagian'");
    }
        $nomor=1;
