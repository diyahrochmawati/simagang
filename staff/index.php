...
if (!isset($_SESSION['usernamestaff'])) {
    ...
    $cekdaftar = mysqli_query($connection, "select * from pendaftaran where status='Menunggu Konfirm SDM'");
    $jumcekdaftar = mysqli_num_rows($cekdaftar);
    
    $ceksurat = mysqli_query($connection, "select * from pendaftaran where status='Menunggu ACC SDM' and surat_acc!=''");
    $jumceksurat = mysqli_num_rows($ceksurat);
    
    $ceknilai = mysqli_query($connection, "select * from pendaftaran where formnilai != '' and status='Menunggu ACC NIlai'");
        ...
        <div class="bg-2 shadow position-fixed" id="sidebar-wrapper">
            <ul class="nav flex-column">
               <li id="produk" class="nav-item mt-4 nav-font border-bottom  ">
                ...
                <li  id="produk" class="nav-item mt-3 nav-font border-bottom">
                    <a class="nav-link text-light p-2" href="?page=nilai"><i class="fas fa-clipboard-list fa-lg"></i>&nbsp&nbsp&nbspPenilaian 
