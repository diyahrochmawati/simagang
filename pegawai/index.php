...
if (!isset($_SESSION['usernameadmin'])) {
    ...
    $cekdaftar = mysqli_query($connection, "select * from pendaftaran where status='Menunggu Konfirmasi Admin'");
    $jumcekdaftar = mysqli_num_rows($cekdaftar);
    
    $ceksurat = mysqli_query($connection, "select * from pendaftaran where status='Magang Diterima' and surat_acc=''");
    $jumceksurat = mysqli_num_rows($ceksurat);
    
    $ceklapor = mysqli_query($connection, "select * from pendaftaran where (status='Verifikasi Laporan Admin' or  status='Silahkan Revisi Laporan') and laporan !='' and formnilai !=''");
    $jumceklapor = mysqli_num_rows($ceklapor);
    
    $ceklegal = mysqli_query($connection, "select * from pendaftaran where status='Legalitas Laporan'");
    $jumceklegal = mysqli_num_rows($ceklegal);
?>
<!DOCTYPE html>
<html>
        ...
        <div class="bg-2 shadow position-fixed" id="sidebar-wrapper">
            <ul class="nav flex-column">
               <li id="produk" class="nav-item mt-4 nav-font border-bottom  ">
                    <a class="nav-link text-light p-2" href="?page=home"><i class="fas fa-home fa-lg"></i>&nbsp&nbsp&nbspKonfirm Pendaftaran  
                ...
                <li  id="produk" class="nav-item mt-3 nav-font border-bottom">
                    <a class="nav-link text-light p-2" href="?page=accadmin"><i class="fas fa-envelope-open-text fa-lg"></i>&nbsp&nbsp&nbspUpload Surat 
                ...
                <li  id="produk" class="nav-item mt-3 nav-font border-bottom">
                    <a class="nav-link text-light p-2" href="?page=laporan"><i class="fas fa-check-circle  fa-lg"></i>&nbsp&nbsp&nbspLaporan & Nilai 
                    <?php if( $jumceklapor != 0){ ?><label style="font-size: 13px; width: 20px; border-radius:50px; background-color: #D7D7D7; color: #2E5C5C; text-align: center;"><?php echo $jumceklapor; ?></label><?php } ?></a>
                </li>
                <li  id="produk" class="nav-item mt-3 nav-font border-bottom">
                    <a class="nav-link text-light p-2" href="?page=legalitas"><i class="fas fa-stamp fa-lg"></i>&nbsp&nbsp&nbspLegalitas Laporan 
                    <?php if( $jumceklegal != 0){ ?><label style="font-size: 13px; width: 20px; border-radius:50px; background-color: #D7D7D7; color: #2E5C5C; text-align: center;"><?php echo $jumceklegal; ?></label><?php } ?></a>
                </li>
                ...
                <li  id="produk" class="nav-item mt-3 nav-font border-bottom ">
                    <a class="nav-link text-light p-2" href="?page=pegawai"><i class="fas fa-user-tie fa-lg"></i>&nbsp&nbsp&nbspAkun Pejabat</a>
                </li>
            ...
            <div class="container-fluid" style="margin-top: 4.6em;">
                <div class="row">
                   ...
                   if ($page=="home") {
                    ...
                    }elseif ($page=="legalitas"){
                        require_once "legalitas.php";
                    }else {
                        require_once "home.php"; 
                    }
                    ?>
