...
<table class="table table-striped table-bordered text-center">
    ...
    <tbody>
        ...
        if(isset($_POST["cari"])){
            ...
            $filter=$_POST["filter"];
                if ($filter=="id_daftar"){
                $data=mysqli_query($connection,"select * from pendaftaran where id_daftar like '%$cari%' and status='Menunggu konfirm KABAG SDM'");
                }elseif($filter=="instansi"){
                $data=mysqli_query($connection,"select * from pendaftaran where instansi like '%$cari%' and status='Menunggu konfirm KABAG SDM'");    
                }else{
                $data=mysqli_query($connection,"select * from pendaftaran where nama_ketua like '%$cari%' and status='Menunggu konfirm KABAG SDM'");   
                }       
            }else{
            $data=mysqli_query($connection,"select * from pendaftaran where status='Menunggu Konfirm KABAG SDM'");
        }
        $no=1;
...
<table class="table table-striped table-bordered text-center">
    <thead class="thead-dark">
        <tr>
            <th>No<th>ID Daftar<th>Status<th>Nama<th>Instansi<th>Jurusan<th>Tgl Magang<th>Proposal<th>Detail
        </tr>
    </thead>
    <tbody>
        <?php
        ...
        if(isset($_POST["cari2"])){
            ...
            $filter2=$_POST["filter2"];
                ...
                $data3=mysqli_query($connection,"select * from pendaftaran where (status='Menunggu Konfirmasi Admin' or status='Menunggu Konfirm SDM' or status='Menunggu Konfirm BAGIAN') and id_daftar like '%$cari2%'");
                }elseif($filter2=="instansi"){
                $data3=mysqli_query($connection,"select * from pendaftaran where (status='Menunggu Konfirmasi Admin' or status='Menunggu Konfirm SDM' or status='Menunggu Konfirm BAGIAN ') and instansi like '%$cari2%'");    
                }elseif($filter2=="nama"){
                $data3=mysqli_query($connection,"sselect * from pendaftaran where (status='Menunggu Konfirm Admin' or status='Menunggu Konfirm SDM' or status='Menunggu Konfirm BAGIAN') and nama_ketua like '%$cari2%'");    
                }else{
                $data3=mysqli_query($connection,"select * from pendaftaran where (status='Menunggu Konfirm Admin' or status='Menunggu Konfirm SDM' or status='Menunggu Konfirm BAGIAN') and status like '%$cari2%'");   
                }
            }else{    
        $data3=mysqli_query($connection,"select * from pendaftaran where status='Menunggu Konfirmasi Admin' or status='Menunggu Konfirm SDM' or status='Menunggu Konfirm BAGIAN'");
    }
        $nomor=1;
