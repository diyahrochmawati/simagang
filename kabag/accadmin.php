...
<table class="table table-striped table-bordered text-center">
    <thead class="thead-dark">
        <tr>
            <th>No<th>Nama<th>Status<th>Jumlah Peserta<th>Instansi<th>Jurusan<th>Tgl Magang<th>Surat<th>Verifikasi Surat<th>Detail
        </tr>
    </thead>
    <tbody>
       <?php
        ...
        if(isset($_POST["cari"])){
            ...
            $filter=$_POST["filter"];
                if ($filter=="posisi"){
                $data3=mysqli_query($connection,"select * from pendaftaran where posisi like '%$cari%' and status='Menunggu ACC KABAG SDM'");
                }elseif($filter=="instansi"){
                $data3=mysqli_query($connection,"select * from pendaftaran where instansi like '%$cari%' and status='Menunggu ACC KABAG SDM'");   
                }else{
                $data3=mysqli_query($connection,"select * from pendaftaran where nama_ketua like '%$cari%' and status='Menunggu ACC KABAG SDM'"); 
                }       
            }else{
            $data3=mysqli_query($connection,"select * from pendaftaran where status='Menunggu ACC KABAG SDM'");
        }
        $nomor=1;
        ...
        <tr>
           ...
           <td><a href="proses_verifikasi.php?user=<?php echo $data4['id_daftar']; ?>" id="btnverifikasi"
            class=" btn btn-info badge-info" data-target="#verifikasi">Verifikasi</a></td>
...
<table class="table table-striped table-bordered text-center">
    <thead class="thead-dark">
        <tr>
            <th>No<th>Nama<th>Status<th>Instansi<th>Posisi<th>Tgl Magang<th>Surat<th>Detail
        </tr>
    </thead>
    <tbody>
       <?php
        ...
        if(isset($_POST["cari2"])){
            ...
            $filter2=$_POST["filter2"];
                ...
                $data=mysqli_query($connection,"select * from pendaftaran where (status='Menunggu ACC SDM' or status='ACC Magang') and id_daftar like '%$cari2%'");
        ...
        $data=mysqli_query($connection,"select * from pendaftaran where status='Menunggu ACC SDM' or status='ACC Magang'");
    }
    $no=1;
